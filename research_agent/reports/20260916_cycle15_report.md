...
## 追記9 (2026-09-18、h-backlog-adaptive-compression-budgetを実装・検証 — バックログ問題自体が解消済みと判明)

最後の仮説`h-backlog-adaptive-compression-budget`(翻訳キューが詰まっているときは圧縮を強める)に着手しました。

まず「読めない断片」指標を定義する必要がありましたが、既存の`readability_metrics.py`(Netflix/BBC基準でCPS違反を判定)と`subtitle_formatter.py`(実際の表示時間を計算)を組み合わせるだけで、新規ロジックなしに実装できました。90件の既存実験に適用したところ、**旧rpm=9時代の`llm_course_ep8_guest_talk_10min`では41.5%のキャプションが表示時間違反**という、この仮説の前提が確かに実在した証拠が見つかりました。

次にbacklog対応の圧縮フラグを実装(キューの深さに応じて文字数予算を最大50%削減、計算式は単体テストで検証済み)し、**あえて密度の高い講義クリップ**(過去にrpm=9でバックログを起こした実績のあるクリップ)でライブ検証しました。

**結果: rpm=60でもrpm=9(意図的に絞った)でも、バックログが再現しませんでした。** avg_mt_latencyはrpm=9の方がむしろ低い(0.109秒 vs rpm=60の0.151秒)という結果で、キューが詰まる兆候は見られませんでした。

**理由: これも4件目の「タイマー修正が副次的に解決していた」パターンです。** 6秒タイマーへの引き上げは、バッチ数そのものを大きく減らしており(旧2.5秒設定では頻繁な小さい呼び出しが大量に発生していた)、結果としてバックログの引き金自体がほぼ消えていました。過去の41.5%違反という数字は旧2.5秒タイマー時代のものです。

**結論: 実装は正しく検証済みですが、今の推奨設定(6秒タイマー)下では発動する場面がほとんどありません。** アイデア自体の否定ではなく、より過酷な負荷(rpm=2-3など)か、より速いスピーチが必要という結論です。

### 今セッションの総括

論文由来3仮説 + 別セッション発案2仮説、合計5つを全て検証しました。**唯一、統計的に意味のある明確な改善を示したのはappend-continuationモード(-25% NE、低コスト)**です。その他4つは、「6秒タイマーへの引き上げ」という今セッション最初の発見が、複数の異なる問題(分断・レイテンシ・信頼度飽和・バックログ)を同時に解決していたことが繰り返し判明する結果となりました。これは偶然ではなく、タイマーの引き上げが翻訳呼び出しの「頻度」そのものを下げたことが、多くの副作用を連鎖的に解消したためです。

**次の一手の候補**: ① append-continuationモードを本番のbest configに正式採用、② semantic-completeness-gatingのさらなるチューニング(min_elapsed/check_interval調整)、③ より過酷な音声(速い話者、雑音混じり)での再検証一式。
