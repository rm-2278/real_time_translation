...
**これで文献由来の3仮説(h-asr-confidence-early-commit, h-monotonic-chunkwise-prompt-enja, h-semantic-completeness-gating)すべて検証完了しました。** どれも「今すぐ採用」には至りませんでしたが、semantic-completeness-gatingは唯一「仕組みが機能した上で、実際に品質面で明確な改善(NE 3.0→0.0)が見えた」ケースです。レート制限を判定呼び出しにも適用してから再検証すれば、化ける可能性があります。
